...
#         att_tm1 = Variable(torch.zeros(1, self.args.hidden_size), volatile=True)
        ...
        while len(completed_hypotheses) < beam_size and t < self.args.decode_max_time_step:
            ...
            y_tm1 = torch.LongTensor([hyp[-1] for hyp in hypotheses], requaires_grad = True)
            if self.args.cuda:
                y_tm1 = y_tm1.cuda()
            ...
            hyp_scores = torch.FloatTensor(new_hyp_scores, requires_grad = True) # new_hyp_scores[live_hyp_ids]
            if self.args.cuda:
                hyp_scores = hyp_scores.cuda()
...
#         att_tm1 = Variable(new_tensor(batch_size, self.args.hidden_size).zero_(), volatile=True)
        att_tm1 = new_tensor(batch_size, self.args.hidden_size).zero_().detach()
        y_0 = torch.LongTensor([self.vocab.tgt['<s>'] for _ in range(batch_size)], requires_grad = True)
...
#         att_tm1 = Variable(torch.zeros(1, self.args.hidden_size), requires_grad=False)
        ...
        for i, hyp in enumerate(completed_hypotheses):
            ...
            completed_hypotheses_words.append( tmp_list )
