...
#         att_tm1 = Variable(new_tensor(batch_size, self.args.hidden_size).zero_(), requires_grad=False)
        att_tm1 = new_tensor(batch_size, self.args.hidden_size).zero_().detach()
...
#             att_t = F.tanh(self.att_vec_linear(torch.cat([h_t, ctx_t], 1)))   # E.q. (5)
    ...
    def translate(self, src_sents, beam_size=None, to_word=True):
        ...
        att_tm1 = torch.zeros(1, self.args.hidden_size, requires_grad=True)
            ...
            for i, hyp in enumerate(completed_hypotheses):
                ...
                for w in hyp:
                    if type( w ) == torch.Tensor:
                        w = w.item()
                    tmp_list.append( self.vocab.tgt.id2word[w] )
                completed_hypotheses[i] = tmp_list
...
#         att_tm1 = Variable(new_tensor(batch_size, self.args.hidden_size).zero_(), volatile=True)
        att_tm1 = new_tensor(batch_size, self.args.hidden_size).zero_().requires_grad_()
...
def train(args):
    ...
    f_loss = open( args.save_to + ".losses", "w" )
    ...
    while True:
        ...
        for src_sents, tgt_sents in data_iter(train_data, batch_size=args.batch_size):
            ...
            if train_iter % args.log_every == 0:
                ...
                f_loss.write( "%d %.2f %.2f\n" % (train_iter, report_loss / report_examples), np.exp(report_loss / report_tgt_words) )
...
#                         symlin_file_abs_path = os.path.abspath(args.save_to + '.bin')
                        symlin_file = args.save_to + '.bin'
                        os.system('ln -sf %s %s' % (model_file, symlin_file))
                else:
                    patience += 1
