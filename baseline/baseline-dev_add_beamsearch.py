...
from keras.layers import Input, Embedding, LSTM, Dense,                             Lambda, Concatenate, Bidirectional
...
def simple_seq2seq( vocab_size_list, language_list,
    ...
    _, forward_state_h, forward_state_c,         backward_state_h, backward_state_c = encoder( encoder_input_emb )
...
#             sentence = K.utils.to_categorical( sentence, input_vocab_size )
        ...
        while init != "<END>" and cnt <= max_len + 1:
            indeces = np.array( [word_to_idx_dict[lan_list[1]][x] for x in idx_list] ).reshape( 3, 1 )
#             index = tile(np.array( [word_to_idx_dict[lan_list[1]][init]] ).reshape( ( 1, 1 ) ), (3,1))
#             if not EMB:
#                 index = [K.utils.to_categorical( index, output_vocab_size )]
            indeces, state_h, state_c = decoder_model.predict( indeces + state )
            state = [state_h, state_c]
            new_sentences = []
            for i in range( 3 ):
#                 indeces, state_h, state_c = decoder_model.predict( [i] + state )
                idx_list = []
                p = []
                for _ in range( 3 ):
                    idx = np.argmax( indeces[i, -1, :] )
                    p.append( indeces[i, -1, idx] )
                    idx_list.append(idx)
                    indeces[i, -1, idx] = -10000
                for j in range(3):
                    init = idx_to_word_dict[lan_list[1]][idx_list[j]]
                    cur_sentence = copy.deepcopy( sentence[i][0] )
                    cur_sentence.append( init )
                    new_sentences.append( [cur_sentence, sentence[i][1] * p[j]] )
            sentences = sorted(new_sentences, key=lambda x : x[1], reverse = True )[0:3]
            cnt += 1
        print( sentences[0][0] )
#         print( words[:-1] )
        f.write( ' '.join( sentences[0][0] ) + "\n" )
    f.close()
